...
public sealed class MainViewModel : NotifyObject
{
    ...
    public string DisclaimerBody => Text(
        "WinFaceUnlock 会安装登录相关系统组件。安装前请确认你理解：\n\n1. 当前功能仍应先在虚拟机中验证，再用于真实登录环境。\n2. 安装过程需要管理员权限，并会注册 Windows 服务和 Credential Provider。\n3. 安装程序不会在此步骤采集人脸、保存 Windows 密码或配置摄像头。\n4. 后续配置面板会单独引导人脸录入、凭据绑定和认证测试。",
        "WinFaceUnlock installs system components related to Windows sign-in. Before continuing, confirm that you understand:\n\n1. This functionality should be validated in a VM before use on a real sign-in environment.\n2. Setup requires administrator privileges and registers a Windows Service and Credential Provider.\n3. This setup step does not capture faces, store Windows passwords, or configure cameras.\n4. The later configuration panel will guide face enrollment, credential binding, and authentication tests.");
