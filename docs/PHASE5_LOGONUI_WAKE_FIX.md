...
- `AutoWakeOnAdvise=true`，LogonUI 加载 Provider 后后台启动人脸登录。
- `TileVisibility=hidden-until-ready`，认证完成前不展示 WinFaceUnlock 磁贴。
- 认证成功后，Provider 通过 `CredentialsChanged` 触发重新枚举，并只在凭据材料就绪时返回自动登录 credential。
- 认证失败时，不显示或选中 WinFaceUnlock 磁贴，LogonUI 保持在 Windows 原生 PIN/密码登录路径。
- Windows PIN、密码、Windows Hello 不会被修改。

如果要在虚拟机中调试可见磁贴，显式开启：

```powershell
.\installer_cli.exe install-provider --provider-binary C:\WinFaceUnlock\windows_provider.dll --show-tile-before-ready
```
...
- 遮挡摄像头或识别失败时，不应选中 WinFaceUnlock 磁贴；用户应仍停留在 PIN/密码兜底登录路径。
- 人脸认证失败后，不应进入立即重试循环。
