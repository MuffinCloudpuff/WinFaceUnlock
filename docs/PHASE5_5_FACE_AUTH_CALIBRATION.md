...
- 2026-06-21：用户反馈一次 LogonUI 自动解锁没有成功。服务日志确认摄像头已打开并完成三次自动认证尝试，但每次 30 帧均被
  `MatchBelowThreshold` 拒绝；同一环境此前成功分数约 `0.76` 到
  `0.79`。本轮判断为光线/姿态导致的匹配分不足，暂不通过降低阈值绕过，后续纳入校准与采样复盘。
