...
& dotnet build $SolutionPath -c $Configuration -p:Platform=x64 --ignore-failed-sources
if ($LASTEXITCODE -ne 0) {
    throw "WinUI setup build failed with exit code $LASTEXITCODE"
}

& dotnet publish $SetupAppProjectPath -c $Configuration -p:Platform=x64 -r win-x64 --self-contained true --ignore-failed-sources
if ($LASTEXITCODE -ne 0) {
